...
initial_date_value=""
final_date_value=""
script_state="DEBUG_ERROR" #state machine. values=START, SHOW_PRODUCT_CONFIG,EDIT_PRODUCT_CONFIG,EDIT_START_PRODUCT_CONFIG,EDIT_CHECK_TMP_PRODUCT_CONFIG,DELETE_USER_CONFIG, DEBUG_ERROR
...
def PrintBarMenu():
	...
	if(language=="SP"):
		Start_Link="<a href='/cgi-bin/products.py?STATE=START' title='Start'><img width='127' height='20' border='0' alt='Main' src='./html_common/Product_Main_Button_SP.png'></a>"
		Show_Link="<a href='/cgi-bin/products.py?STATE=SHOW_PRODUCT_CONFIG' title='Show'><img width='127' height='20' border='0' alt='Start' src='./html_common/Product_Show_Button_SP.png'></a>"
		Edit_Link="<a href='/cgi-bin/products.py?STATE=EDIT_START_PRODUCT_CONFIG' title='Edit'><img width='127' height='20' border='0' alt='Start' src='./html_common/Product_Edit_Button_SP.png'></a>"
	else:
...
def CheckProdConfigFileIntegrity(file_to_check):
	...
	if (os.path.isfile(configfiles_path+file_to_check)==False):return -2
		...
		if(Debug_Flag=="DEEP"):print "<br>--Starting validation process for tokens"  		
		...
		for line in open(configfiles_path+file_to_check):
			...
			if (Product_token=="" or Product_token=="None"):
				print "<br><b><font color=red>ERROR - item in column Product cannot be empty. Line="+str(count-1)+". Product config file="+configfiles_path+file_to_check+"</font></b>"				
				if (Debug_Flag=="DEEP"):print "<br><b>Finished Function CheckProdConfigFileIntegrity. Returning=-20</b>"	
				return -20

			if (N_token=="" or N_token=="None"):			
				print "<br><b><font color=red>ERROR - item in column N cannot be empty. Line="+str(count-1)+". Product config file="+configfiles_path+file_to_check+"</font></b>"				
				if (Debug_Flag=="DEEP"):print "<br><b>Finished Function CheckProdConfigFileIntegrity. Returning=-30</b>"	
				return -30

			if (Code_token=="" or Code_token=="None"):			
				print "<br><b><font color=red>ERROR - item in column Code cannot be empty. Line="+str(count-1)+". Product config file="+configfiles_path+file_to_check+"</font></b>"				
				if (Debug_Flag=="DEEP"):print "<br><b>Finished Function CheckProdConfigFileIntegrity. Returning=-40</b>"	
				return -40			

			if (TargetWeight_token=="" or TargetWeight_token=="None"):			
				print "<br><b><font color=red>ERROR - item in column TargetWeight cannot empty. Line="+str(count-1)+". Product config file="+configfiles_path+file_to_check+"</font></b>"				
				if (Debug_Flag=="DEEP"):print "<br><b>Finished Function CheckProdConfigFileIntegrity. Returning=-50</b>"	
				return -50
			else:
				if (count>1):
					try:	
						...
						if(test<=0):
							print "<br><b><font color=red>ERROR - item in column TargetWeight is not a positive number="+str(TargetWeight_token)+". Line="+str(count-1)+"</font></b>"
							if (Debug_Flag=="DEEP"):print "<br><b>Finished Function CheckProdConfigFileIntegrity. Returning=-50</b>"
							return -50
			...
			if (MaxTolerance_token=="" or MaxTolerance_token=="None"):			
				print "<br><b><font color=red>ERROR - item in column MaxTolerance cannot be empty. Line="+str(count-1)+". Product config file="+configfiles_path+file_to_check+"</font></b>"				
				if (Debug_Flag=="DEEP"):print "<br><b>Finished Function CheckProdConfigFileIntegrity. Returning=-60</b>"	
				return -60

			else:
				if (count>1):
					try:	
						if (Debug_Flag=="DEEP"):print "<br>DEBUG - Testing the float cast="+str(MaxTolerance_token)
						test=float(MaxTolerance_token)

						if(test<=0):
							print "<br><b><font color=red>ERROR - item in column MaxTolerance is not a positive number="+str(MaxTolerance_token)+". Line="+str(count-1)+"</font></b>"
							if (Debug_Flag=="DEEP"):print "<br><b>Finished Function CheckProdConfigFileIntegrity. Returning=-60</b>"
							return -60
			...
			if (MinTolerance_token=="" or MinTolerance_token=="None"):			
				print "<br><b><font color=red>ERROR - item in column MinTolerance cannot be empty. Line="+str(count-1)+". Product config file="+configfiles_path+file_to_check+"</font></b>"				
				if (Debug_Flag=="DEEP"):print "<br><b>Finished Function CheckProdConfigFileIntegrity. Returning=-70</b>"	
				return -70

			else:
				if (count>1):
					try:	
						if (Debug_Flag=="DEEP"):print "<br>DEBUG - Testing the float cast="+str(MinTolerance_token)
						test=float(MinTolerance_token)

						if(test<=0):
							print "<br><b><font color=red>ERROR - item in column MinTolerance is not a positive number="+str(MinTolerance_token)+". Line="+str(count-1)+"</font></b>"
							if (Debug_Flag=="DEEP"):print "<br><b>Finished Function CheckProdConfigFileIntegrity. Returning=-50</b>"
							return -70
...
def PrintTable(editable):
	...
	if(language=="SP"):print "<br><b><font color='green'>Archivo de configuracion actual</font></b>"
	else:print "<br><b><font color='green'>Current product config file</font></b>"
	...
	#printing in editable mode - EDITABLE = TRUE
		...
		if(language=="SP"):print "</td><td><b><font color='blue'>Archivo de configuracion (modo editable)</font></b>"
		else:print "</td><td><b><font color='blue'>Edit config file</font></b>"
	
		print "<form action='/cgi-bin/products.py?STATE=EDIT_PRODUCT_CONFIG' method='POST'>"
		
		print "<table border='1'>"
		if(language=="SP"):print "<tr><td>N</td><td>Producto</td><td>Codigo</td><td>Peso Meta(Kg)</td><td>Tolerancia max(%)</td><td>Tolerancia min(%)</td><td><font color='red'><b>Agregar/Borrar linea</b></font></td><td><font color='orange'><b>Mensajes</b></font></td></tr>"
		else:print "<tr><td>N</td><td>Product</td><td>Code</td><td>TargetWeight(Kg)</td><td>MaxTolerance(%)</td><td>MinTolerance(%)</td><td><font color='red'><b>Delete line?</b></font></td><td><font color='orange'><b>Warnings</b></font></td></tr>"			
		...
		for line in open(configfiles_path+product_config_filename_tmp):
			...
			count+=1
			list=line.split(',')
			if(Debug_Flag=="DEEP"):print "<br>Debug -"+str(list)		
			...
			if (count>1):	
				...
				if(language=="SP"):delete_line_button_value="borrar_linea"
				else:delete_line_button_value="delete_line" 
		...
		if(language=="SP"):print "<td><input type='submit' name='add_newline_button' value='agregar_linea'></td>"
		else:print "<td><input type='submit' name='add_newline_button' value='add_newline'></td>"
		...
		if(language=="SP"):
			#FIXME: enable this button #print "<tr><td><input type='submit' name='revert_cfgfile_button' value='Recuperar anterior' title='Revert to old product config file'></td>"
			print "<tr><td></td>" #place future Recover-old-file button here
			print "<td><input type='submit' name='save_tmp_changes_button' value='Guardar temp' title='Save changes to temporary product config'>" #buton to save changes 
			print "<input type='submit' name='discard_changes_button' value='Cancelar' title='Cancel changes'>"#button to cancel	
			print "<input type='submit' name='apply_changes_button' value='Aplicar' title='Apply changes'></td></tr>"#button to publish	
		...
		#add
		if((add_newline_configfile_action=="add_newline" and language=="ENG") or (add_newline_configfile_action=="agregar_linea" and language=="SP")):
		...
		#save changes
		if((save_tmp_configfile_action=="Save temp" and language=="ENG") or (save_tmp_configfile_action=="Guardar temp" and language=="SP")):
			if(Debug_Flag=="DEEP"):print "<br>--Button save activated!"
			#os.system("ls >> config_files/example.csv")	#for debug purposes
			SaveTempConfFile(count)

		#apply changes
		if((apply_tmp_configfile_action=="Apply" and language=="ENG") or (apply_tmp_configfile_action=="Aplicar" and language=="SP")):
			if(Debug_Flag=="DEEP"):print "<br>--Button apply activated!"
		...
		for w in range(2,count+1):
			...
			if ((button_form.getvalue(delete_button_names[w-2])=="borrar_linea" and language=="SP") or(button_form.getvalue(delete_button_names[w-2])=="delete_line" and language=="ENG") ):
...
def SaveTempConfFile(number_of_lines):
	...
	if(language=="SP"):command_string="echo 'N,Product,Code,TargetWeight(Kg),MaxTolerance(%),MinTolerance(%)' > "+configfiles_path+product_config_filename_tmp
	else:command_string="echo 'N,Producto,Codigo,PesoMeta(Kg),Tolerancia max(%),Tolerancia min(%)' > "+configfiles_path+product_config_filename_tmp
	...
	if (CheckProdConfigFileIntegrity(product_config_filename_tmp)==0):
		...
		RedirectToUrl("EDIT_CHECK_TMP_PRODUCT_CONFIG") # force check since tmp file has errors
...
def Main():
	...
	elif (script_state=="EDIT_CHECK_TMP_PRODUCT_CONFIG"):
		if (Debug_Flag=="DEEP"):print "<br>--Try to open product config file(edit mode)="+configfiles_path+product_config_filename
		if (os.path.isfile(configfiles_path+product_config_filename)==True):
			PrintTable("TRUE")
			CheckProdConfigFileIntegrity(product_config_filename_tmp)
...
if(language=="SP"):print "<h2>Productos</h2>"
else:print "<h2>Products</h2>"
